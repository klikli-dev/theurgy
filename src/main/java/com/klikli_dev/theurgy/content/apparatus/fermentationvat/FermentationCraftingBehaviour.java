    ...
    protected boolean craft(RecipeHolder<FermentationRecipe> pRecipe) {
        ...
        for (var ingredient : pRecipe.value().getIngredients()) {
            for (int i = 0; i < this.inputInventorySupplier.get().size(); i++) {
                if (ingredient.test(ItemUtil.getStack(this.inputInventorySupplier.get(), i))) {
                    try (var tx = Transaction.openRoot()) {
                        this.inputInventorySupplier.get().extract(ItemResource.of(ItemUtil.getStack(this.inputInventorySupplier.get(), i)), this.getIngredientCount(pRecipe), tx);
                        tx.commit();
                    }
                    break;
                }
