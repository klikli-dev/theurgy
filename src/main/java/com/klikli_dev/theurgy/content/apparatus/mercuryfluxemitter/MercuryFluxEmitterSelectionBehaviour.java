...
public class MercuryFluxEmitterSelectionBehaviour extends SelectionBehaviour<MercuryFluxEmitterSelectedPoint> {
    ...
    public boolean canCreate(Level level, BlockPos pos, BlockState state) {
        ...
        return level.getCapability(CapabilityRegistry.MERCURY_FLUX_HANDLER, pos, state, null, null) != null || level.getCapability(Capabilities.Energy.BLOCK, pos, state, null, null) != null;
    }

    @Override
    protected MercuryFluxEmitterSelectedPoint create(Level level, BlockPos pos, BlockState state) {
        if (!this.canCreate(level, pos, state)) return null;
        if (!this.selectedPoints.isEmpty()) this.selectedPoints.clear();
