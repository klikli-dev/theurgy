...
public class Color {
    ...
    public Color setRed(float r) {
        return this.ensureMutable().setRedUnchecked((int) (0xff * Mth.clamp(r, 0, 1)));
    }
    ...
    public Color setGreen(float g) {
        return this.ensureMutable().setGreenUnchecked((int) (0xff * Mth.clamp(g, 0, 1)));
    }
    ...
    public Color setBlue(float b) {
        return this.ensureMutable().setBlueUnchecked((int) (0xff * Mth.clamp(b, 0, 1)));
    }
    ...
    public Color setAlpha(float a) {
        return this.ensureMutable().setAlphaUnchecked((int) (0xff * Mth.clamp(a, 0, 1)));
    }
    ...
    public Vec3 asVector() {
        return new Vec3(this.getRedAsFloat(), this.getGreenAsFloat(), this.getBlueAsFloat());
    }
    ...
    public Color scaleAlpha(float factor) {
        return this.ensureMutable().setAlphaUnchecked((int) (this.getAlpha() * Mth.clamp(factor, 0, 1)));
    }

    // ********* //

    public Color mixWith(Color other, float weight) {
        return this.ensureMutable()
                .setRedUnchecked((int) (this.getRed() + (other.getRed() - this.getRed()) * weight))
                .setGreenUnchecked((int) (this.getGreen() + (other.getGreen() - this.getGreen()) * weight))
                .setBlueUnchecked((int) (this.getBlue() + (other.getBlue() - this.getBlue()) * weight))
                .setAlphaUnchecked((int) (this.getAlpha() + (other.getAlpha() - this.getAlpha()) * weight));
    }

    public Color darker() {
        int a = this.getAlpha();
        return this.ensureMutable().mixWith(BLACK, .25f).setAlphaUnchecked(a);
    }

    public Color brighter() {
        int a = this.getAlpha();
        return this.ensureMutable().mixWith(WHITE, .25f).setAlphaUnchecked(a);
    }
    ...
    public Color modifyValue(UnaryOperator<Integer> function) {
        int newValue = function.apply(this.value);
